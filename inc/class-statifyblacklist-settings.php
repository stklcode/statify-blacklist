...
class StatifyBlacklist_Settings extends StatifyBlacklist {
	...
	public static function register_settings() {
		register_setting( 'statify-blacklist', 'statify-blacklist', array( __CLASS__, 'sanitize_options' ) );
		...
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-referer',
			array( 'label_for' => 'statifyblacklist-referer-active' )
		);
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-referer',
			array( 'label_for' => 'statifyblacklist-referer-cron' )
		);
		add_settings_field(
		...
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-target',
			array( 'label_for' => 'statifyblacklist-target-active' )
		);
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-target',
			array( 'label_for' => 'statifyblacklist-target-cron' )
		);
		add_settings_field(
		...
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-ip',
			array( 'label_for' => 'statifyblacklist-ip-active' )
		);
		add_settings_field(
		...
		add_settings_field(
			...
			'statify-blacklist',
			'statifyblacklist-ua',
			array( 'label_for' => 'statifyblacklist-ua-active' )
		);
		add_settings_field(
	...
	public static function sanitize_options( $options ) {
		// Extract referer URLs.
		if ( empty( trim( $options['referer']['blacklist'] ) ) ) {
			$referer = array();
		} else {
		...
		if ( empty( trim( $options['target']['blacklist'] ) ) ) {
			$target = array();
		} else {
			$target = array_filter(
				array_map(
					...
					explode( "\r\n", str_replace( '\\\\', '\\', $options['target']['blacklist'] ) )
				),
				function ( $a ) {
					return ! empty( $a );
				}
		...
		// Extract IP array.
		if ( empty( trim( $options['ip']['blacklist'] ) ) ) {
			$ip = array();
		} else {
			$ip = array_filter(
				array_map(
		...
		if ( empty( trim( $options['ua']['blacklist'] ) ) ) {
			...
			$ua = array_filter(
				array_map(
					...
					explode( "\r\n", str_replace( '\\\\', '\\', $options['ua']['blacklist'] ) )
				),
				function ( $a ) {
					return ! empty( $a );
				}
		...
		$referer_given   = $res['referer']['blacklist'];
		$referer_invalid = array();
		if ( StatifyBlacklist::MODE_NORMAL === $res['referer']['regexp'] ) {
			// Sanitize URLs and remove empty inputs.
			$referer_sanitized = self::sanitize_urls( $referer_given );
		} elseif ( StatifyBlacklist::MODE_REGEX === $res['referer']['regexp'] || StatifyBlacklist::MODE_REGEX_CI === $res['referer']['regexp'] ) {
		...
		$referer_diff                = array_diff( $referer_given, $referer_sanitized );
		$res['referer']['blacklist'] = $referer_sanitized;

		// Sanitize target list.
		$target_given   = $res['target']['blacklist'];
		$target_invalid = array();
		if ( StatifyBlacklist::MODE_REGEX === $res['target']['regexp'] || StatifyBlacklist::MODE_REGEX_CI === $res['target']['regexp'] ) {
